...
in
{
  ...
  options.services.nextcloud = {
    ...
    webserver = {
      user = mkOption {
        type = types.str;
        description = "The user that runs the webserver for nextcloud. Used to configure phpfpm `listen.user` setting.";
      };
      group = mkOption {
        type = types.str;
        description = "The group that runs the webserver for nextcloud. Used to configure phpfpm `listen.group` setting.";
      };
    };
    ...
    nginx = {
      configure = mkOption {
        type = types.bool;
        default = true;
        description = "Configure nginx as a webserver for nextcloud";
      };
  ...
  config = mkIf cfg.enable (mkMerge [
...

    {
        ...
        pools.nextcloud = {
          ...
          settings =
            mapAttrs (name: mkDefault) {
              "listen.owner" = cfg.webserver.user;
              "listen.group" = cfg.webserver.group;
            }
            // cfg.poolSettings;
      ...
      services.nginx = (lib.mkIf cfg.nginx.configure {
